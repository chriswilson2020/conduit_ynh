...
// contact, deal (since Task 7), or (since Phase 3 Task 6) a project, so a
// request naming zero or more than one of the four is a client error, not an
// empty-filter no-op.
const listQuerySchema = z
  .object({
    company_id: z.uuid().optional(), contact_id: z.uuid().optional(), deal_id: z.uuid().optional(),
    project_id: z.uuid().optional(),
  })
  .refine((v) => [v.company_id, v.contact_id, v.deal_id, v.project_id].filter((x) => x !== undefined).length === 1, {
    message: "exactly one of company_id, contact_id, deal_id or project_id is required",
  });
