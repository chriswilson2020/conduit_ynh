 ...
 * the inbox stops being a three-pane grid and becomes three screens visited one
 * at a time, and "which screen am I on, what is it called, and how do I leave
 * it" is the whole of that behaviour. Kept here it is testable without a DOM --
 ...
 * both of its neighbours are one control away from it in each direction. That
 * is what makes "no surface is a dead end" true here -- every level has a
 * leading control except the hub, and the hub is the one screen the bottom
 * navigation itself can always reach.
 */
 ...
 * before this phase. */
const ALL_PANES: Readonly<Record<InboxLevel, boolean>> = {
 ...
 */
const DESKTOP_VIEW: InboxStackView = {
