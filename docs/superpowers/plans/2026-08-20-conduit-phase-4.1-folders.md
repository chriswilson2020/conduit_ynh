...
**DONE — what Task 5 inherits.** The EXPLAIN was run at 20k threads / 85k messages and the "no new indexes" expectation did NOT hold: `mail_messages (folder, thread_id)` is now in 0005 (hand-written, named query + measurements in the migration; 70.8ms/123k buffers -> 6.6ms/290 on the worst case, index-only probes everywhere else). Three contract points the web layer keys on: (1) `trash`/`archive` accept at most **50** threadIds per request, 400 above that, while `hide` keeps 200 — and a proxy 504 on those two means the ANSWER was lost, not the action, so refetch rather than retry; (2) the new SSE family is `[["mail-folders", accountId]]`, published by the folder PATCH and by a discovery pass that creates or reclassifies a folder; (3) unread counting excludes each account's trash_folder in the badge AND the list's per-row `unread` flag, but NOT in `?byFolder=1` (whose rows carry no accountId, so folder names collapse across accounts — a per-account sidebar badge needs an account-scoped variant this release does not have).
