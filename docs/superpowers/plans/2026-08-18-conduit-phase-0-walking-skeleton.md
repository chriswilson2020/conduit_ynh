...
// max: 2, not 1 — a single test occasionally issues two queries concurrently, and with
// max: 1 the second would queue behind the first. Isolation between test files does not
// come from connection limits, though: it relies on `fileParallelism: false` in
// vitest.config.ts. If that ever gets flipped on for speed, this stops being a loud
// error and becomes silent cross-file data races.
 ...
 * so any serial primary key a later phase adds resets too.
 *
...
Do **not** hardcode `TRUNCATE TABLE users`. `CASCADE` only reaches tables with a foreign-key path
back to `users`, so a later table with no such path would silently escape truncation and leak rows
between tests — a failure that surfaces as "this test sees rows it didn't create", a long way from
its cause.

Note `db.execute()` returns postgres.js's `RowList`, which is the row array itself, not a
`{ rows: [...] }` wrapper.
