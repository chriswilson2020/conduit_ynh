...
>
> **As built.** schema.ts: `meetings` :628-676 (CHECK `meetings_has_link` :675, spelled `num_nonnulls(...) >= 1` so it reads as one family with notes'/files' `exactlyOne` at :249 — reachability WHY at :657-668), `meetingAttendees` :700-712 (CHECK :711, no `created_at` by design :687-693), `events.meetingId`/`.mailThreadId` :301/:310 (both `references((): AnyPgColumn => ...)` — forward-declared targets, the `pipelines.projectId` device), verb CHECK :319-322. Migration `0008_long_strong_guy.sql`: generated 1-40, hand-written partial uniques 41-66 (0004's `mail_accounts_user_email_active_unique` terms), no-backfill rationale 67-76. Shared: `eventVerbSchema` :1122, event pointers :141/:151 (required-and-nullable, deliberately not optional), `attendeeIdentityFields` :1335, `attendeeExactlyOne` :1350, `meetingAttendeeSchema` :1357, `meetingAttendeeInputSchema` :1370, `meetingAtLeastOneLink` :1385, `meetingSchema` :1389, `meetingInputShape` :1420 (NOT exported), create :1433, update :1452, list filters :1467.
>
> **The drill** (schema.test.ts:951) pins its premise three ways before migrate: both tables absent, both events columns absent, and `met` REJECTED by the 0007 CHECK. Post-migrate: tables exist, the pre-existing event survived with both pointers NULL, new rows reference pre-migration parents, `met` inserts, `convened` still rejects, and both partial uniques are present in `pg_indexes` with the right predicate — that last specifically because they live in no snapshot, so only a from-the-files migration proves the .sql still carries them. Reviewer-verified by mutation that the drill applies the shipped SQL (removing `met` from the migration's CHECK fails it).
>
...
> - **`meetingSchema` carries a REQUIRED `attendees` array** (:1412, not in the spec's data model). Every meeting-returning path — create, get, update, archive, unarchive, and every `listMeetings` row — must hydrate attendees or the client's `parseWith` throws. There is no read path that wants the bare row.
> - **`updateMeeting` must re-assert `meetingAtLeastOneLink` against the MERGED row** and raise a 4xx: the refine deliberately does not ride `meetingUpdateInputSchema` (:1443-1452, `taskDatesPaired`'s precedent — a patch sees one snapshot, never the stored row), so clearing the last link would otherwise surface as a 500 from the CHECK. `meetingAtLeastOneLink` is module-private: export it or re-derive.
...
> - Attendees replace as a SET, never patched row-by-row (`meetingAttendeeInputSchema` carries no `id`, :1364-1369). Duplicate-attendee inserts surface as Postgres `23505` — remap to `ConflictError` the way mail-accounts.ts does for its partial unique.
> - **No FK indexes on `meetings`' four record columns, nor on `meeting_attendees.contact_id`.** The partial unique `(meeting_id, contact_id)` leads with `meeting_id`, so it cannot serve `listMeetings`' contact-attendance EXISTS lookup by contact id. Matches the closest precedent (notes/files/events carry none; only mail got them in 0004/0005). 0008 is UNSHIPPED and editable in place — add indexes only if Task 2 MEASURES the need (EXPLAIN, house rule), preserving the journal timestamp and converging conduit_test via psql.
> - Both event pointers on one row are legal at schema level; nothing in Phase 5 writes such a row (pinned as column shape, not use case, schema.test.ts:1195-1201).
...
- `timeline.tsx`: add the three verbs to `VERB_BADGE` (the map is exhaustively typed against `eventVerbSchema`, so the compiler names them) per spec Amendment 1's ruling: `met` = **M**, `mail_received` = **I**, `mail_sent` = **T**, and `dependency_removed` moves **M -> X**. Task 1 landed the spec's original letters and correctly flagged that two collided; the amendment resolves it. Update the two stale head comments in timeline.tsx that still claim the badges are unique. A `met` entry links to its meeting; a mail entry renders the derived `mailSubject` and links to the thread.
