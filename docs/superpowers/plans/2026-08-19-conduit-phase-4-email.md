...
| `packages/api/src/services/mail-sync.ts` | `AccountSync` state machine, `SyncManager` lifecycle |
| `packages/api/src/services/mail-imap.ts` | `ImapClient`/`SyncClock`/`SyncLogger`/`IngestMessageFn` contract, re-exported from mail-sync.ts (added, not in the original table) |
...
| `packages/web/src/pages/settings-mail-lib.ts` | settings-mail form state, validation, the four password-convention submission shapes (added, not in the original table) |
...
| `packages/web/src/components/mail/mail-lib.ts` | recipient parsing/template substitution, inbox page accumulation, reply/forward rules, `MESSAGE_FRAME_SANDBOX`, the CSP builder (added, not in the original table; renamed from `composer-lib.ts`) |
| `packages/web/src/components/mail/rich-text.tsx` | shared TipTap editor (`StarterKit.configure({ link: false })` + its own configured `Link`) (added, not in the original table) |
| `packages/web/src/components/rail/{rail,mail}.tsx` | Mail tab on the existing rail (AS BUILT: replaces this table's original `{contact,company,deal,project}-detail.tsx` "+ Mail tab" row — Task 10 put the tab on the shared rail component the four detail pages already mount, not on each page individually) |
...
`mail-ingest.ts`: `ingestMessage(db, dataDir, { accountId, folder, uid, raw, flags })` — ONE transaction:
...
- **SSE coalescing** is a batch-boundary decision, not a per-message one: ingest publishes its own `[["mail-threads"], ["mail-thread", id], ["mail-unread"]]` hint per new message, so a 50-message backfill batch fires 150 key hints. **SUPERSEDED by the ruling immediately below** — this line originally speculated that, if per-message hints proved noisy in the web layer, batch-end coalescing (one hint per pass) would be the fix rather than suppressing ingest's own hint. Task 5 measured the actual cost and decided against adding any coalescing at all; left here only as the discarded alternative, not as live guidance.
...
`mail-imapflow.ts`: `ImapflowClient implements ImapClient` — imapflow with `secure`/STARTTLS per `imap_security`, mailbox locks around ops, SEARCH SINCE for the backfill filter, `client.idle()` with AbortSignal race. Also export `imapVerify(settings)` / `smtpVerify(settings)` (nodemailer `verify()`) and wire them as the real deps of Task 3's testConnection. Unit tests, as built: 32 tests across 8 describes — more than construction alone, since the adapter's own translation layer (option mapping, falsy-return handling, error normalisation, the walk cache) turned out to be unit-testable behind a mocked imapflow client; the real network/TLS/protocol behaviour still comes only from Task 8's integration suite.
...
- **Accepted payload limit (v0.5.0): the thread-detail route returns EVERY message in the thread**
  ...
  opening. Nothing in a normal CRM conversation comes close, which is why this ships as-is; the
  v0.5.x fix is a server-side cap (newest N messages) plus a `truncated` flag the conversation renders as
...
`e2e/mail.spec.ts`, serial, runId names. Seed via IMAP APPEND from the test (imapflow as a root devDependency for e2e, `tls: { rejectUnauthorized: false }`, CI Dovecot host/creds from Task 8's `E2E_MAIL_*` env): a 3-message thread (2 inbound from `alice@example.com` + 1 unrelated message). Journey: create contact Alice with that email → Settings → add mail account via Dovecot preset, then overwrite host/port fields from the env values → test connection green → wait for sync (poll inbox until `thread-row` count == 2) → thread auto-linked to Alice (link chip visible) → open thread, mark-read clears unread badge → create a deal for Alice, reopen thread, deal suggestion appears, click links it → reply ("Thanks Alice") → Mailpit API (request via `page.request`) shows the message → sent message renders in conversation → unlinked filter hides the linked thread, shows the unrelated one → search finds the thread by body text. Existing 33 e2e untouched.

**AS BUILT (20 Aug).** `e2e/mail.spec.ts`, 9 tests covering all ten journey steps, ~3.5s of the e2e job's 24s; 42 e2e green (the 33 that existed are untouched). `imapflow` is a root devDependency, resolved on the server. Five things worth carrying forward:

- **Retry safety needed one thing run-id scoping cannot give.** Nothing empties the Dovecot mailbox and nothing resets the database between Playwright retries, so a second attempt starts with the first attempt's mail in INBOX and its rows in the database. Every subject/address/body marker carries the run id and every assertion matches a run-id-scoped SET, as planned — but a leftover ACCOUNT is different in kind: two live accounts on one mailbox each ingest every message into the same (globally threaded) conversation, so the retry would see every message twice. `beforeAll` therefore archives any active own account before seeding.
- **The unread badge is asserted as a DELTA.** It counts every unread thread in the installation, which on a retry includes the previous attempt's, so the only sound claim is "down by exactly the one thread this step opened" — read before, `toHaveText(before - 1)` after, with absent-at-zero handled. The per-row unread dot carries the run-id-scoped half of that assertion.
- **Waiting for the SECOND message before opening the thread is load-bearing.** A conversation marks itself read once per mount, so a message ingested moments after that would leave the thread unread with nothing to re-mark it. The step waits for the html message's marker to reach the row's SNIPPET (which is the newest message's) before clicking, and the badge assertion additionally polls across reloads, since a reload remounts the conversation and re-marks it.
- **One step is not the UI, because there is no UI for it.** A deal's contact is read-only everywhere it is shown and the board's New deal dialog takes a title and a value, so `deals.contact_id` is reachable only through `PATCH /api/deals/:id` today. The deal itself is created on the board; only that column is set through the API. **A per-deal contact/company picker would close this** — worth a line in a later phase's backlog, not a v0.5.0 change.
- **The plan's "poll until `thread-row` count == 2" was replaced by set membership**, for the same retry reason: the inbox may legitimately hold other threads. `pollWithReload` (reload, retry, short per-assertion timeouts) is the shape used wherever a background sync pass is what is being waited for — the inbox is live over SSE and these normally pass first time, but a spec that could only learn about a pass through one hint would turn a lost hint into a silent multi-minute hang.

Also in this task, from the Task 8 review: the Dovecot image is pinned (`2.3.21.1`, which is what `2.3-latest` resolved to — the config it is handed is 2.3 syntax and 2.4 renamed enough of it to break boot); `wait-for-mail.mjs` asserts that 1144 advertises STARTTLS and 1143 does not (the property the two-instance split exists for, which a config edit could otherwise hollow out invisibly); the e2e job's failure step takes the unit job's `doveadm mailbox list` dump.

**For Task 12:** the e2e job's `Provision the mail credential key` step (`openssl rand -out data/mail.key 32`) is the CI stand-in for what `scripts/install` must do for a real deployment — the journey adds a mail account, so that file is now genuinely exercised on every run, and an install path that forgets it fails at the first account with a 503. The e2e job also cannot drop its Dovecot/Mailpit containers now: e2e/mail.spec.ts needs both.

### Task 12: Packaging + release 0.5.0

`scripts/install`: after data_dir setup — `openssl rand -out "$data_dir/mail.key" 32`, `chmod 600`, `chown $app:$app` (match the existing file-provisioning idiom in the script). `scripts/upgrade`: same, guarded by `[ -f ]`. `scripts/backup`/`restore`: mail.key rides the existing data_dir coverage — VERIFY that data_dir is fully included (it is for files); add an explicit line only if the scripts enumerate paths. Config: **deviation, as built** — no `MAIL_KEY_PATH` env was added to the systemd service/config template. `config.ts` already falls back to `$DATA_DIR/mail.key` when the var is unset, `.env` already carries `DATA_DIR`, and `config.test.ts` pins that default, so an explicit `MAIL_KEY_PATH` entry would only be a second copy of the same path to keep in sync — deliberately left out (see `f71b73f`). Also in this task (Chris's decision, 19 Aug): merge branch `fix/fastify-static-advisories` (commit 901f205, @fastify/static v8 -> v10 security migration, verified against v0.4.3) into the phase branch before the version bump — regenerate the lockfile on the server after the merge, re-run the full suite, mention the migration in the release notes, and drop the `conduit_test_secfix` database on the dev server. Then Phase 2 Task 10 release mechanics verbatim (bump all versions to 0.5.0, CI gate, ff-merge to main, tag v0.5.0, Release workflow builds the asset, manifest sha update on main, branch cleanup, hand Chris the one sudo upgrade command). Live verification checklist: add real account (Chris's local Dovecot via preset), watch sync populate, link a deal, send a reply from the CRM and see it in his normal mail client's Sent.

**AS BUILT.** The `fix/fastify-static-advisories` merge (`b10587e`) landed with a lockfile defect: a conflicted merge commits the index, not the working tree, so the lockfile `scripts/remote.sh npm install` regenerated on the server never made it into `b10587e` — and the mail-key provisioning commit that followed it (`f71b73f`) inherited the same stale lockfile. Both are non-installable on their own (`npm ci` refuses the `@fastify/static` version mismatch). Fixed forward in `a76d00a` rather than by amending history, per the house precedent `be6f4dd` set earlier in this phase for the same class of conflicted-merge lockfile defect: the branch tip is what CI gates and what gets tagged, and it is correct from there on; this was accepted into main on that basis at review.

Packaging shipped as: `scripts/install` mints `$data_dir/mail.key` unconditionally after data_dir setup; `scripts/upgrade` mints it only when absent (the `[ -f ]` guard — see the reworded comment on that guard's actual contract, landed in the pre-release polish commit below); `scripts/restore` re-asserts 600 on an existing key and mints one when restoring a pre-0.5.0 backup, whose database has no mail accounts yet for a fresh key to strand; `scripts/backup` needed no change, since it already declares `$data_dir` whole rather than enumerating paths under it. No `MAIL_KEY_PATH` was added anywhere — the deviation recorded in the paragraph above.

The `chore: pre-release polish for v0.5.0` commit adds two more Task 12 items directly: the boot-time `mail.key`-existence warning in `server.ts` (makes the spec's Key-handling promise literally true at boot, not just documented) and the `manifest.toml` `disk` bump from `500M` to `2G` (mail attachments now persist into `data_dir` across a 90-day backfill; `500M` was sized pre-mail). It lands just after the `chore(ci): typecheck the e2e specs` commit (`2de5f76`), which closed a gap where `tsc -b`'s project references never covered `e2e/*.spec.ts` at all -- Playwright strips types without checking them, so the specs had never been typechecked by anything -- by adding `e2e/tsconfig.json` and folding `tsc --noEmit -p e2e` into the `typecheck` script. Both housekeeping items from the merge paragraph above are confirmed done on the dev server as of this commit: no `conduit_test_secfix` database remains (`psql -lqt` shows none), and no stray `node`/`vitest`/`tsx`/`npm` process was left running.
