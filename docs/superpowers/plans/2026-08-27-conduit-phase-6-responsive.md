...
- **AN UNREPRODUCIBLE UNIT FAILURE, RECORDED RATHER THAN EXPLAINED.** Two server-side `vitest run`s during Task 6's mutation window reported `1 failed | 1828 passed` and named no test (the reporter's failure block was never captured). **Ten runs since are clean at 1829 -- five with a dev server up against `conduit_dev` on the same Postgres cluster the api tests use for `conduit_test`, plus a 1s request loop against `/api/companies`, `/api/gantt` and `/api/tasks`, and five with the machine quiet** -- as is every CI `test` job on the branch. The load hypothesis is therefore NOT supported: it was the condition that held during the original window, and it produced 5/5 green.

  It is also not attributable to that window's mutated tree. `chart.tsx`'s `appliedScrollZoomRef` guard is named by no unit test at all -- which is exactly why the quality reviewer's mutation of it survived -- and `GRID_MAX_HEIGHT` is read in one place, an inline `maxHeight` style, asserted by nothing. Neither could have produced a unit failure. `vitest.config.ts` collects `packages/*/src/**/*.test.ts` only, so nothing under `e2e/` is ever in that suite either.

  **Left open.** If it returns, the suite's three real budgets are where to look first, and all three survived the contended runs: `scheduling.test.ts:154` (5s around a 50-hop `shiftTask` cascade -- the only wall clock in the suite over a DATABASE-bound operation, every hop a round trip inside a transaction holding `lockSiblingGroup`); `mail-send.test.ts:724` (a file-descriptor DELTA with a slack of 5 explicitly reserved for "a pooled database connection", so it moves with the pool rather than the clock); and `mail-ingest.test.ts:371` (3s over a CPU-only guard whose real path is sub-millisecond -- the same class as `mail-content.test.ts:290`, which the coordinator measured at max 1ms under 24 burners on 10 cores). No fix was made, because there is nothing yet to fix: a speculative change here would be a guess dressed as maintenance.
